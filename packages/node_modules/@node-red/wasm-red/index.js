...
module.exports = {
    ...
    wasmwrap : function(node) {
        ...
        for(let C = 1; C <= WASMCOUNT; C++) {
            ...
            let cmd = `${thisdir}/bin/javy compile ${f}.bundle.inner.${C}.js -o ${f}.inner.${C}.wasm`;
    ...
    wasmwrap_code: function(node, ops = {}) {
       ...
       // tmp = `/tmp/${tmp}`;
        ...
        if(ops.spider) {
            // Use SpiderMonkey instead
            
        }


        console.log("Compiling Wasm file", `${tmp}.bundle.inner.js`);
        // Make exec sync
        let sync = true;
        ...
        // if ops.optimize is true we use wasm-opt
        ...
        if(ops.optimize) {
            ...
            console.log(`Optimized from ${size} to ${size_opt} bytes (${size_opt/size*100}%)`);
        }
        // hash the node code
        ...
        WebAssembly.compile(
            ...
            console.log(`Compiled wasm to cache ${name}`);
            GLOBAL_CACHE[name] = module;
        });
