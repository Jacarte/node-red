...
/**
 * Blacklist to avoid wrapping nodes
 */
BLACKLIST = [
    ["input", "function"],
    ["*", "debug"],
...
/**
 * White list to allow wrapping nodes
 */
WHITELIST = [
    ["input", "lower-case"],
];

function inferContructorTrace() {
    var myError = new Error();
    var trace = myError.stack.split('\n');

    for(let i = 0; i < trace.length; i++){
        if(trace[i].includes("createNode")){
            // Then the previous one is the location 
...
// This is a super ugly solution, we need to pass the node.file variable in the constructor
function getConstructorFolder(trace){
    // at new LowerCaseNode (/Users/javierca/Documents/Develop/node-red/wasm-nodes/sandtrap-node-lowercase/lower-case.js:8:14)
    let file = trace.split(" ");
...
Node.prototype.on = function(event, callback) {
    var node = this;
    var wasm_red = require("@node-red/wasm-red")
    
    if (WHITELIST.some(([e, t]) => (e === event && t === node.type))) {
        let folder = getConstructorFolder(inferContructorTrace());
        console.log("Whitelisted", node.id, node.type)
        // This is cached, second time should be fast
        let code_start = process.hrtime();
        let start = process.hrtime();
        let [_, callback_file] = wasm_red.wasmwrap_code(callback.toString(), ops = {
            cwd: folder,
            function_body: true,
        ...
        // console.log("Wrapped", callback_file)
        // Get the cb
        let r = require(callback_file);
        r = r.__esModule ? r.default : r

        callback = r(() => {
            code_start = process.hrtime();
        },
        // report time
        () => {
            let duration = process.hrtime(code_start);
            console.log("Wasm called in", duration[0] * 1000 + duration[1] / 1000000, "ms")
            // TODO send the time for gathering
            return duration[0] * 1000 + duration[1] / 1000000
        },
        wasm_red.GLOBAL_CACHE, node).bind(this);
        /*let cb = import(callback_file);
        callback = cb;*/
        let duration = process.hrtime(start);
        console.log("Wrapped in", duration[0] * 1000 + duration[1] / 1000000, "ms")
    }
    // console.log(node.id, node.file, node.type)
    if (event == "close") {
