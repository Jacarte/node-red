...
module.exports = function(RED) {
                ...
                for (var n=0; n < msgs[m].length; n++) {
                    ...
                    if (msg !== null && msg !== undefined) {
                        if (typeof msg === 'object' && !Buffer.isBuffer(msg) && !util.isArray(msg)) {
                            ...
                            node.error(RED._("functionp.error.non-message-returned",{ type: type }));
                        }
                    }
                }
            }
    ...
    function FunctionNode(n) {
        ...
        if (RED.settings.functionExternalModules === false && node.libs.length > 0) {
            throw new Error(RED._("functionp.error.externalModuleNotAllowed"));
        }
            ...
            __node__: {
                ...
                on: function() {
                    if (arguments[0] === "input") {
                        throw new Error(RED._("functionp.error.inputListener"));
                    }
        ...
        if (node.hasOwnProperty("libs")) {
            ...
            modules.forEach(module => {
                ...
                if (vname && (vname !== "")) {
                    if (sandbox.hasOwnProperty(vname) || vname === 'node') {
                        node.error(RED._("functionp.error.moduleNameError",{name:vname}))
                        moduleErrors = true;
                        return;
                    }
                    sandbox[vname] = null;
                    ...
                    if (spec && (spec !== "")) {
                        moduleLoadPromises.push(RED.import(module.module).then(lib => {
                            ...
                            node.error(RED._("functionp.error.moduleLoadError",{module:module.spec, error:err.toString()}))
                            throw err;
                        }));
                    }
                }
            ...
            if (moduleErrors) {
               throw new Error(RED._("functionp.error.externalModuleLoadError"));
           }
        }
        const RESOLVING = 0;
        const RESOLVED = 1;
        ...
        Promise.all(moduleLoadPromises).then(() => {
            ...
            try {
                ...
                processMessage = function (msg, send, done) {
                    ...
                    context.results.then(function(results) {
                        ...
                        fs.appendFileSync('times.txt', converted + "\n");
                        console.log("duration: " + converted)
            ...
            node.error(RED._("functionp.error.externalModuleLoadError"));
        });
    }
    RED.nodes.registerType("functionp",FunctionNode, {
        dynamicModuleList: "libs",
        settings: {
            functionExternalModules: { value: true, exportable: true }
        }
    });
    RED.library.register("functionsp");
};
