...
var wasm_node = require("@node-red/wasm-red")
...
/*const WHITELIST = [
    ...
    //"06-httpproxy.js"
    // "05-tls.js"
    // "89-delay.js", we need to define setInterval
    // "21-debug.js", we need to define httpAdmin
    // "90-exec.js", wont work due to isolation :(
    // "89-trigger.js", weird behaviour, does not wait
    // "101-functionp.js", we need to port register from RED.
    // "23-watch.js", Javy error, `Err` value: unhandled tag: -8', crates/quickjs-wasm-rs/src/js_binding/value.rs:428:44
    // "80-template.js", resolve and register

    
    // http ones
    // We need to implement them all :(
    // Working nodes !
    ...
    "18-sort.js",
    "17-split.js",
    "19-batch.js" // setInterval is not defined
    
]*/

// Read the WHITELIST from the ENV var
// eg export WHITELIST="10-function.js,20-inject.js" && npm ...

let WHITELIST = process.env.WHITELIST ? process.env.WHITELIST : "MANIFEST.json";
WHITELIST = JSON.parse(fs.readFileSync(
    WHITELIST
));
...
function loadNodeSet(node) {
    ...
    try {
        ...
        // check if file name is in the whitelist
        let keys = Object.keys(WHITELIST);
        let key;

        if(keys.includes(path.basename(node.file)))
            key = path.basename(node.file);
        else if (keys.includes(path.resolve(node.file)))
            key = path.resolve(node.file);

        if(key){
            console.log("Wasm wrapping", node.file);
            
            let content = fs.readFileSync(node.file, "utf8");
            var [_, r] = wasm_node.wasmwrap_code(content, ops={
                cwd: path.dirname(node.file),
                full_node: true,
                file: node.file
            }, 
            api_permissions = WHITELIST[key].API,
            package_permissions = WHITELIST[key].PACKAGES,
            file = WHITELIST[key].PACKAGES,
            file_permissions = WHITELIST[key].FILE_PERMISSIONS,
            );

            
            r = require(r);
        } else {
            //console.log("Normal node loading")
        ...
        if (typeof r === "function") {
            ...
            // Setting wasm_red here
            red.wasm_red = require("@node-red/wasm-red");
            red.GLOBAL_CACHE = wasm_node.GLOBAL_CACHE;  
