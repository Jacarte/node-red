...
function createNodeApi(node) {
    ...
    /*const old = runtime.nodes.createNode;


    runtime.nodes.createNode = function(context, RED) {
        // console.log("Creating node for", context);
        const oldon = context.on.bind(context);
        context.on = function(event, listener) {
            // console.log(listener.toString(), context.type);

            if(event == "input"){
                listener = (msg, send, done) => {
                    console.log("Wasm call", msg)
                }
            }
        }.bind(context)
